...
if( empty( $_SESSION['iduser'] ) ){
	...
	if(isset($_REQUEST['submit'])){
		...
		if(empty($password)){
			$query = "UPDATE petugas SET username='$username',admin='$admin',fullname='$fullname' WHERE iduser='$iduser'";
		} else {
			$query = "UPDATE petugas SET username='$username',password='$password',admin='$admin',fullname='$fullname' WHERE iduser='$iduser'";
		}
		...
		$sql = mysqli_query($koneksi,"SELECT * FROM petugas WHERE iduser='$id'");
