...
if( empty( $_SESSION['iduser'] ) ){
	...
	if( isset( $_REQUEST['submit'] )){
		...
		list($nis,$siswa,$a,$idkelas) = mysqli_fetch_array($sql);
		
		echo '<div class="alert alert-danger">Yakin akan menghapus Siswa:';
		echo '<br>Nama  : <strong>'.$siswa.'</strong>';
		echo '<br>NIS   : '.$nis;
		
		$qprodi = mysqli_query($koneksi,"SELECT kelas FROM kelas WHERE idkelas='$idkelas'");
		list($kelas) = mysqli_fetch_array($qprodi);
		
		echo '<br>Kelas : '.$kelas.' ('.$idkelas.')<br><br>';
