...
if( empty( $_SESSION['iduser'] ) ){
	...
	if( isset( $_REQUEST['sub'] )){
		...
		if(isset($_REQUEST['submit'])){
			...
			$sql = mysqli_query($koneksi,"SELECT password FROM petugas WHERE iduser='$iduser' AND password='$pass1'");
			if(mysqli_num_rows($sql) > 0){
				$do = mysqli_query("UPDATE petugas SET password='$pass2' WHERE iduser='$iduser'");
				if($sql > 0){
